<form class="md:w-1/2 space-y-5" wire:submit.prevent='editOffer'>
    <div>
    ...
    <div>
        ...
        <x-text-input id="image" accept="image/*" class="block mt-1 w-full text-gray-600" type="file" wire:model="new_image" />
        <div class="my-5 w-80">
            <x-input-label  :value="__('Actual image')" />
            <img src="{{asset('storage/offers/' . $image )}}" alt="{{'Offer image ' . $title}}">
        </div>
        <div class="my-5 w-80">
            @if ($new_image)
               <span class="'block text-sm text-gray-500 font-bold uppercase mb-2'">New Image:</span>
